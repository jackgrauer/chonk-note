...
pub async fn extract_to_string(_pdf_path: &Path, _page_num: usize) -> Result<String> {
    // Text extraction removed - use pdfalto or leptess methods instead
    Ok(String::from("Text extraction not available in string mode"))
}
...
// Stub implementations - TODO: implement with pdfalto and leptess
async fn extract_pdfalto_method(
    pdf_path: &Path,
    page_num: usize,
    width: usize,
    ...
    let xml_path = temp_file.path();

    // Run pdfalto to extract text as XML
    let page_arg = format!("{}", page_num + 1); // pdfalto uses 1-indexed pages
    ...
    // Simple XML parsing - extract text from String elements
    let mut grid = vec![vec![' '; width]; height];
    let mut row = 0;
    let mut col = 0;

    // Extract all CONTENT="..." text with TextLine breaks
    let mut pos = 0;
    while let Some(start) = xml_content[pos..].find("CONTENT=\"") {
        let abs_start = pos + start + 9;
        if let Some(end) = xml_content[abs_start..].find('"') {
            let text = &xml_content[abs_start..abs_start + end];
            ...
            // Place text in grid with word spacing
            if col > 0 && col < width {
                grid[row][col] = ' ';
                col += 1;
            }
        ...
        // Check for line break after this word
        if let Some(textline_end) = xml_content[pos..pos.saturating_add(200).min(xml_content.len())].find("</TextLine>") {
            if textline_end < 200 {  // TextLine close is nearby
                row += 1;
                col = 0;
                if row >= height {
                    break;
...
async fn extract_leptess_ocr_method(
    ...
    let page_arg = format!("{}", page_num + 1); // pdftoppm uses 1-indexed pages
...
pub fn get_page_count(pdf_path: &Path) -> Result<usize> {
    ...
    if !output.status.success() {
        return Ok(1); // Default to 1 page if pdfinfo fails
    }
    ...
    Ok(1) // Default to 1 page if parsing fails
}